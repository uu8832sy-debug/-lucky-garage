...
async function renderCommerceProducts(){
  try{const snap=await getDocs(collection(db,"shops",SHOP_ID,"products"));products=snap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.visible!==false&&p.approvedForJerry===true).sort((a,b)=>Number(a.order||999)-Number(b.order||999));renderCards();}catch(err){console.warn("commerce products",err);}
}
injectModals();
