...
async function start() {
  if (!codeForm || !codeInput || !verifyBtn || !entryView || !garageView || !resultView) {
    ...
    document.body.insertAdjacentHTML("afterbegin", `<div style="position:fixed;z-index:99999;inset:12px 12px auto;padding:14px;border-radius:12px;background:#7f1d1d;color:#fff;font-weight:800">網站檔案版本不一致，請重新上傳 v20 完整穩定版。</div>`);
    return;
  }
